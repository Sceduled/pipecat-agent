...
async def run_outbound(
    ...
    async def on_client_connected(_transport, _client):
        ...
        # Clear pre-synth PCM — we won't inject it through the pipeline
        # (raw frame injection corrupts LLMAssistantAggregator's TTS-speaking state)
        lead_context.pop("opener_pcm", None)

        # Wait for TTS WebSocket using the CORRECT library State enum.
        # NOTE: websockets.protocol.State.OPEN — NOT starlette's WebSocketState
        # which has no .OPEN attribute and always evaluates False.
        ...
        logger.info(f"TTS ready in ~{elapsed:.1f}s. Speaking opener: {opener}")
