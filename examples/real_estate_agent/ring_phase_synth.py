...
# ---------------------------------------------------------------------------
# Sarvam REST API call — fast (<1s) and locked to exact live TTS prosody
# ---------------------------------------------------------------------------
...
async def synthesize_opener(
    *,
    ...
    try:
        ...
        if raw[:4] == b"RIFF":
            raw = raw[44:]
        logger.info(f"ring_phase_synth: Synthesized {len(raw)} PCM bytes via REST for opener in <1s")
        return raw
    except Exception as e:
