"""
Outbound Call Agent ΓÇö Prestige Realty
...
Three outbound call types (set via call_type in the /dial payload):
  follow_up     ΓÇö Lead submitted a form, following up on their enquiry
  visit_reminder ΓÇö Remind about an upcoming site visit
  new_launch    ΓÇö Inform a past lead about a new property matching their criteria
"""
...
# ---------------------------------------------------------------------------
# Session store ΓÇö maps session_token ΓåÆ lead context dict
...
# ---------------------------------------------------------------------------
# System prompts ΓÇö one per call type
# ---------------------------------------------------------------------------
...
- CRITICAL FOR LATENCY: ALWAYS start your response with a short conversational filler ending in a comma (e.g., "Got it, ", "Sure, ", "Okay, ", "Right, "). This allows the audio to start playing instantly while you think.
...
TTS PRONUNCIATION RULES ΓÇö follow these exactly for natural phone audio:
...
def build_outbound_prompt(call_type: str, lead: dict) -> str:
    ...
    if call_type == "follow_up":
        return f"""You are Shubh, a confident and knowledgeable real estate advisor for Prestige Realty in Bangalore.
...
3. Understand their requirement ΓÇö location, BHK, budget.
...
1. Introduce yourself warmly and confirm you are speaking with {name}.
2. Confirm their visit naturally ΓÇö mention the date, time, and property.
...
# ---------------------------------------------------------------------------
# Dial helper ΓÇö calls Vobiz REST API to initiate an outbound call
# ---------------------------------------------------------------------------
...
async def run_outbound(
    ...
    outbound_directive = (
        ...
        f"You MUST introduce yourself immediately, state the reason for your call, and proactively lead the conversation.\n"
        f"Do not wait for them to ask questions; YOU called THEM."
    )
    
    # We combine the UI-configured persona with the dynamic lead context
    system_prompt_final = (
        f"You are representing: {company_name}\n\n"
        f"--- YOUR PERSONA & STYLE ---\n"
        f"{system_prompt}\n\n"
        f"--- OUTBOUND CALL DIRECTIVE ---\n"
        ...
        f"Visit Date: {lead_context.get('visit_date', '')} {lead_context.get('visit_time', '')}\n"
        f"Property: {lead_context.get('property_name', '')}\n\n"
        f"{_BASE_RULES}"
    )
    ...
    # before synthesizing ΓåÆ each short sentence = one synthesis job = smooth audio.
    ...
    async def on_client_connected(_transport, _client):
        ...
        # We manually add the full opener to the context so the LLM remembers it.
        context.add_message({"role": "assistant", "content": f"{opener_part1} {opener_part2}"})
        
        # To achieve < 2.0s startup latency, we push "Hello?" immediately.
        # We then wait 1.5 seconds before pushing the rest of the sentence. 
        # This prevents jamming the Sarvam WebSocket while it is initializing, 
        # allowing "Hello?" to synthesize and stream to the user instantly.
        async def send_greetings():
            await asyncio.sleep(0.05)
            logger.info(f"Queuing opener part 1: {opener_part1}")
            await worker.queue_frames([TTSSpeakFrame(text=opener_part1)])
            
            # Wait for WS to connect and Hello to play
            await asyncio.sleep(1.5) 
            logger.info(f"Queuing opener part 2: {opener_part2}")
            await worker.queue_frames([TTSSpeakFrame(text=opener_part2)])
            
        asyncio.create_task(send_greetings())
